#define SOUFFLE_GENERATOR_VERSION "5dfac36de"
...
if (performIO) {
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"fact-dir","./input"},{"inc-delete","false"},{"inc-insert","false"},{"incDelta","false"},{"name","edge"},{"operation","input"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
if (!inputDirectory.empty()) {directiveMap["fact-dir"] = inputDirectory;}
{
FunctionTimer timer("reading relation rel_edge_04d25f5060e9043b");
...
auto untypedTuple = UntypedTuple::fromTypedTuple("path",tuple);
auto*& ruleSet = DerivationManager::untypedTuple2RuleApplications[untypedTuple];
if (ruleSet == nullptr) {
ruleSet = new std::set<RuleApplication>();
}
std::map<std::string, souffle::RamDomain> varValues{};
varValues.insert({"x", ramBitCast(env0[0])});
varValues.insert({"y", ramBitCast(env0[1])});
...
if( !((rel_path_76cf4ab0e06a645b->contains(Tuple<RamDomain,2>{{ramBitCast(env0[0]),ramBitCast(env1[1])}},READ_OP_CONTEXT(rel_path_76cf4ab0e06a645b_op_ctxt)))&& ((DerivationManager::ruleAppExistsInCompleteSet(UntypedTuple::fromTypedTuple("path", Tuple<RamDomain,2>{{ramBitCast(env0[0]),ramBitCast(env1[1])}}),RuleApplication{2,{{"x",ramBitCast(env0[0])},{"y",ramBitCast(env1[1])},{"z",ramBitCast(env0[1])}}}))))) {
...
std::map<std::string, souffle::RamDomain> varValues{};
varValues.insert({"x", ramBitCast(env0[0])});
varValues.insert({"y", ramBitCast(env1[1])});
varValues.insert({"z", ramBitCast(env0[1])});
...
rel_new_path_a0453b38c2806942->purge();
if (performIO) {
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"inc-delete","false"},{"inc-insert","false"},{"incDelta","false"},{"name","path"},{"operation","output"},{"output-dir","./output"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
{
FunctionTimer timer("writing relation rel_path_76cf4ab0e06a645b");
...
namespace  souffle {
using namespace souffle;
class Sf_test_full: public SouffleProgram {
public:
 Sf_test_full();
 ~Sf_test_full();
void run();
...
void Sf_test_full::runFunction(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
...
// -- query evaluation --
{
FunctionTimer timer("stratum_edge");
 std::vector<RamDomain> args, ret;
stratum_edge_31e40d0d94fe9f7c.run(args, ret);
}
{
FunctionTimer timer("stratum_path");
 std::vector<RamDomain> args, ret;
...
void Sf_test_full::run(){
runFunction("", "", false, false);
}

void Sf_test_full::runAll(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
runFunction(inputDirectoryArg, outputDirectoryArg, performIOArg, pruneImdtRelsArg);
}

void Sf_test_full::printAll([[maybe_unused]] std::string outputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"inc-delete","false"},{"inc-insert","false"},{"incDelta","false"},{"name","path"},{"operation","output"},{"output-dir","./output"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf_test_full::loadAll([[maybe_unused]] std::string inputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"fact-dir","./input"},{"inc-delete","false"},{"inc-insert","false"},{"incDelta","false"},{"name","edge"},{"operation","input"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
SymbolTable& Sf_test_full::getSymbolTable(){
return symTable;
}

RecordTable& Sf_test_full::getRecordTable(){
return recordTable;
}

void Sf_test_full::setNumThreads(std::size_t numThreadsValue){
...
void Sf_test_full::executeSubroutine(std::string name,const std::vector<RamDomain>& args,std::vector<RamDomain>& ret){
if (name == "edge") {
stratum_edge_31e40d0d94fe9f7c.run(args, ret);
return;}
if (name == "path") {
...
namespace souffle {
SouffleProgram *newInstance_test_full(){return new  souffle::Sf_test_full;}
SymbolTable *getST_test_full(SouffleProgram *p){return &reinterpret_cast<souffle::Sf_test_full*>(p)->getSymbolTable();}
} // namespace souffle
...
{
FunctionTimer timer("dumping derivations");
DerivationManager::derivationInfo2JsonFile(opt.getSourceFileName(), "", DerivationManager::untypedTuple2RuleApplications, opt.getOutputFileDir()==""?"./output":opt.getOutputFileDir());
DerivationManager::derivationInfo2JsonFile(opt.getSourceFileName(), "insert", DerivationManager::untypedTuple2DeltaInsertRuleApplications, opt.getOutputFileDir()==""?"./output":opt.getOutputFileDir());
DerivationManager::derivationInfo2JsonFile(opt.getSourceFileName(), "delete", DerivationManager::untypedTuple2DeltaDeleteRuleApplications, opt.getOutputFileDir()==""?"./output":opt.getOutputFileDir());
...
try
    ...
    {
        std::cerr << "Prob calc failed: " << e.what() << std::endl;
    }
return 0;
...
namespace  souffle {
using namespace souffle;
class factory_Sf_test_full: souffle::ProgramFactory {
public:
souffle::SouffleProgram* newInstance();
 factory_Sf_test_full();
private:
};
} // namespace  souffle
namespace  souffle {
using namespace souffle;
souffle::SouffleProgram* factory_Sf_test_full::newInstance(){
return new  souffle::Sf_test_full();
}

 factory_Sf_test_full::factory_Sf_test_full():
souffle::ProgramFactory("test_full"){
}
...
extern "C" {
souffle::factory_Sf_test_full __factory_Sf_test_full_instance;
}
#endif
} // namespace souffle
