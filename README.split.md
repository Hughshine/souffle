...
## 0.3 Full rule set baseline（2026-01-05，det-opt + naive-split）
设置：
...
- rewrite 使用 component-wise FC，并在 `FC_WMC_HYBRID` 里记录 `fc_build_ms`/`wmc_ms`。
- `FC_WMC_HYBRID` 现在从 rewrite 前开始，包含 rewrite + split 的时间。
- `rewrite_s` 来自 `rewrite_ms`（在 `FC_WMC_HYBRID` 中记录）。
- BDD manager 仅在存在 slow components 时初始化；否则 `manager_init_ms=0`。
- 两条 pipeline 都只保存 `needOutput` 节点概率（no‑rewrite 也按 component 应用 evidence），
  避免 `IO_DUMP` 排序内部节点造成偏差。
- rewrite 的 `dd_live_nodes` 为 component 的 live nodes 求和，近似值，**不可**与 no-rewrite 直接对比。
