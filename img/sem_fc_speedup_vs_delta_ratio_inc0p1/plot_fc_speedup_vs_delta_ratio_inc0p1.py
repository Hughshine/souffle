...
INPUT_TSV = BASE_DIR / "fc-sem-delta-ratio.tsv"
OUTPUT_PNG = BASE_DIR / "fc_speedup_vs_delta_ratio_inc0p1_inc0p5_filtered_50ms.png"
OUTPUT_SUMMARY = BASE_DIR / "fc_speedup_vs_delta_ratio_inc0p1_inc0p5_filtered_50ms_summary.tsv"
...
def load_points() -> list[tuple[float, float, str, float, float]]:
    ...
    with INPUT_TSV.open() as f:
        ...
        for row in reader:
            ...
            full = float(row["Full_FC_s"])
            inc = float(row["Inc_FC_s"])
...
def plot_points(points: list[tuple[float, float, str, float, float]]) -> None:
    ...
    ax.set_title("FC Speedup vs DeltaE/|E| (>=0.05s, by turn + ideal)")
