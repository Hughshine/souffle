...
#include "ast2ram/seminaive-with-delta/ClauseTranslator.h"
#include "Global.h"
...
Own<ram::Operation> ClauseTranslator::createInsertion(const ast::Clause& clause) const {
    ...
    auto recordDerivation = mk<ram::RecordDerivation>(
                        ...
                        std::move(cloneVarExprs(varExprs)),
                        false, true);
...
Own<ram::Operation> ClauseTranslator::addNegatedAtomDerived(
    ...
    auto clauseStr = clause.toString();
    return mk<ram::Filter>(
    mk<ram::Negation>(mk<ram::DerivationCheck>(headRelationName, std::move(values),
        context.getClauseNum(&clause), std::move(cloneClauseVarMap(clauseVarMap)),std::move(cloneVarExprs(clauseVarExprs)))),
        std::move(op));
}
