...
Own<ram::Operation> ClauseTranslator::addNegatedAtomDerived(
    ...
    auto clauseStr = clause.toString();
    return mk<ram::Filter>(
    mk<ram::Negation>(mk<ram::DerivationCheck>(headRelationName, std::move(values), context.getClauseNum(&clause), std::move(cloneClauseVarMap(clauseVarMap)), std::move(cloneVarExprs(varExprs)))), std::move(op));
}
