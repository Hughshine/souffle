...
class IncClauseTranslator : public ast2ram::ClauseTranslator {
public:
    IncClauseTranslator(const TranslatorContext& context, TranslationMode mode = Incremental);
    ~IncClauseTranslator();
...
std::vector<Own<ram::Expression>> getClauseVarExprs(const ast::Clause& clause) const;
    ...
    virtual Own<ram::Operation> addNegatedAtomDerived(
                Own<ram::Operation> op, const ast::Clause& clause, const ast::Atom* atom) const;
    virtual Own<ram::Operation> addAtomDerived(
        Own<ram::Operation> op, const ast::Clause& clause, const ast::Atom* atom) const ;
