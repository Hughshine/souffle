...
Own<ram::Expression> IncValueTranslator::translateValue(const ast::Argument* arg) {
    assert(arg != nullptr && "arg should be defined");
    return IncValueTranslator(context, index)(*arg);
}

Own<ram::Expression> IncValueTranslator::visit_(type_identity<ast::Variable>, const ast::Variable& var) {
    if (index.isDefined(var.getName())) {
        return makeRamTupleElement(index.getDefinitionPoint(var.getName()));
    }
    if (index.hasConstantDefinition(var.getName())) {
        return context.translateValue(index, &index.getConstantDefinition(var.getName()));
    }
    assert(false && "variable not grounded");
    return nullptr;
}

Own<ram::Expression> IncValueTranslator::visit_(
...
Own<ram::Expression> IncValueTranslator::visit_(type_identity<ast::NilConstant>, const ast::NilConstant&) {
    return mk<ram::SignedConstant>(0);
}

Own<ram::Expression> IncValueTranslator::visit_(type_identity<ast::TypeCast>, const ast::TypeCast& typeCast) {
...
Own<ram::Expression> IncValueTranslator::visit_(type_identity<ast::Counter>, const ast::Counter&) {
...
Own<ram::Expression> IncValueTranslator::visit_(type_identity<ast::RecordInit>, const ast::RecordInit& init) {
    VecOwn<ram::Expression> values;
...
Own<ram::Expression> IncValueTranslator::visit_(type_identity<ast::BranchInit>, const ast::BranchInit& adt) {
...
Own<ram::Expression> IncValueTranslator::visit_(type_identity<ast::Aggregator>, const ast::Aggregator& agg) {
