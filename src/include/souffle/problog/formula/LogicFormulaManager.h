...
public:
    ...
    explicit LogicNode(int varIndex, const Node& node)
        : type(Type::VAR), varIndex(varIndex), node(node) {}

    explicit LogicNode(int varIndex, const Hyperedge& edge)
        : type(Type::VAR), varIndex(varIndex), edge(edge) {}
        ...
        switch (type) {
            case Type::VAR:
                if (node.has_value()) {
                    ...
                    result = "X" + std::to_string(varIndex);
                }
                break;
            case Type::NOT:
...
public:
    ...
    LogicNodeRef createVar(int index, const Node& node) override {
        auto prototype = std::make_shared<LogicNode>(index, node);
        return LogicNodeRef(getUniqueNode(std::move(prototype)));
    }

    LogicNodeRef createVar(int index, const Hyperedge& edge) override {
        auto prototype = std::make_shared<LogicNode>(index, edge);
        return LogicNodeRef(getUniqueNode(std::move(prototype)));
    }
