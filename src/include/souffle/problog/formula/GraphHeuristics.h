...
private:
    ...
    struct Var {
        ...
        CanonicalKey stable_key; // for detecting duplicates
    };
    ...
    void buildVariables() {
        ...
        // for stable order generation (wrt tuple content, not unstable IDs)
        // --- 修改 1: 对节点按 tuple 内容排序 ---
        // 首先，将所有有效的节点指针收集到一个 vector 中。
        std::vector<NodePtr> sorted_nodes;
        for (const auto& n : view_->getNodes()) if (n) sorted_nodes.push_back(n);
        // 然后，使用节点的 tuple 内容作为键进行排序，而不是使用不稳定的 ID。
        std::sort(sorted_nodes.begin(), sorted_nodes.end(), [](const NodePtr& a, const NodePtr& b) {
            return a->getTuple() < b->getTuple(); // CHANGED: 使用 getTuple() 进行稳定排序
        });


        // Input facts (isFact == true), probability < 1.0, not pruned
        for (const NodePtr& n : sorted_nodes) {
            ...
            if (!idx_of_.count(vid)) {
                ...
                fact_var_to_tuple_[vid] = n->getTuple(); // vid => tuple
            }
        }
        std::vector<EdgePtr> sorted_edges;
        for (const auto& e : view_->getEdges()) if (e) sorted_edges.push_back(e);
        // 使用边的 tuple 内容作为键进行排序。
        std::sort(sorted_edges.begin(), sorted_edges.end(), [](const EdgePtr& a, const EdgePtr& b) {
            return a->getEdgeKey() < b->getEdgeKey(); // CHANGED: 使用 getTuple() 进行稳定排序
        });

        // Rules (edges), probability < 1.0, not pruned
        for (const EdgePtr& e : sorted_edges) {
            if (!e) continue;
            ...
            if (!idx_of_.count(vid)) {
                ...
                rule_var_to_key_[vid] = e->getEdgeKey(); // Add this line
            }
        }
...
//        std::cout << "Variables in current universe:\n";
//        for (const auto& v : vars_) {
//            std::cout << "  var_id=" << v.var_id
//                      << " kind=" << ((v.kind == Var::FACT) ? fact_var_to_tuple_[v.var_id].toString() : std::get<1>(rule_var_to_key_[v.var_id]).toString() ) << "\n";
//        }
    }
    ...
    void frontierCollect(const NodePtr& fact,
        ...
        const auto inE = view_->getIncomingEdgesStable(fact);
    ...
    std::map<UntypedTuple, int> tuple_to_fact_var_;
    ...
    void buildEvents() {
        ...
        // A stably sorted list of edges is used for deterministic iteration.
        std::vector<EdgePtr> sorted_edges;
        for (const auto& e : view_->getEdges()) {
            if (e) sorted_edges.push_back(e);
        }
        std::sort(sorted_edges.begin(), sorted_edges.end(), [](const EdgePtr& a, const EdgePtr& b) {
            return a->getEdgeKey() < b->getEdgeKey();
        });

        // Helper function to create events with canonical keys and var_id orders.
        auto push_event = [&](const std::vector<int>& s, double w) {
            std::vector<int> initial_vids;
            initial_vids.reserve(s.size());
            for (int v : s) if (idx_of_.count(v)) initial_vids.push_back(v);
            std::sort(initial_vids.begin(), initial_vids.end());
            initial_vids.erase(std::unique(initial_vids.begin(), initial_vids.end()), initial_vids.end());

            if (initial_vids.size() < 2) return;

            using KeyVidPair = std::pair<std::variant<UntypedTuple, EdgeKey>, int>;
            std::vector<KeyVidPair> sorted_pairs;
            sorted_pairs.reserve(initial_vids.size());

            for (int vid : initial_vids) {
                if (fact_var_to_tuple_.count(vid)) {
                    sorted_pairs.emplace_back(fact_var_to_tuple_.at(vid), vid);
                } else {
                    sorted_pairs.emplace_back(rule_var_to_key_.at(vid), vid);
                }
            }

            std::sort(sorted_pairs.begin(), sorted_pairs.end(),
                [](const KeyVidPair& a, const KeyVidPair& b) { return a.first < b.first; });

            std::vector<int> t;
            CanonicalKey key;
            t.reserve(sorted_pairs.size());
            key.reserve(sorted_pairs.size());
...
//            for (int vid : t) {
//                if (fact_var_to_tuple_.count(vid)) {
//                    std::cout << "FACT " << " (" << fact_var_to_tuple_.at(vid).toString() << ") ";
//                } else {
//                    std::cout << "RULE " << " (" << std::get<1>(rule_var_to_key_.at(vid)).toString() << ") ";
//                }
//            }
//            std::cout << "\n";
            events_.push_back(Event{std::move(t), w, std::move(key)});
        };

        // OR-events
        {
            std::map<UntypedTuple, std::vector<int>> head_to_flips;
            for (const EdgePtr& e : sorted_edges) {
                if (e->pruned || !(e->getProbability() < 1.0)) continue;
                if (const NodePtr h = view_->getOutput(e)) {
                     head_to_flips[h->getTuple()].push_back(mapEdgeId(e->getId()));
                }
            }
            for (const auto& kv : head_to_flips) {
                if (kv.second.size() >= 2) push_event(kv.second, P_.w_or);
            }
        }
        ...
        for (const EdgePtr& e : sorted_edges) {
            ...
            std::vector<int> vars;
            if (e->getProbability() < 1.0) {
                vars.push_back(mapEdgeId(e->getId()));
            }
...
//                    std::vector<int> sorted_out(out.begin(), out.end());
                    std::sort(sorted_pairs.begin(), sorted_pairs.end(),
                        [](const KeyVidPair& a, const KeyVidPair& b) { return a.first < b.first; });

                    std::vector<int> sorted_out;
                    for (const auto& pair : sorted_pairs) {
                        sorted_out.push_back(pair.second);
                    }
                    vars.insert(vars.end(), sorted_out.begin(), sorted_out.end());
                }
            }
            if (vars.size() >= 2) {
                std::sort(vars.begin(), vars.end(), [this](int a, int b) {
                    // sort by stable key
                    std::variant<UntypedTuple, EdgeKey> ka, kb;
                    if (fact_var_to_tuple_.count(a)) ka = fact_var_to_tuple_.at(a);
                    else                           ka = rule_var_to_key_.at(a);
                    if (fact_var_to_tuple_.count(b)) kb = fact_var_to_tuple_.at(b);
                    else                           kb = rule_var_to_key_.at(b);
                    return ka < kb;
                });
                push_event(vars, P_.w_and);
            }
        }
//
        // BRIDGE (type 1)
        for (const EdgePtr& e : sorted_edges) {
            ...
            for (size_t i = 0; i < inputs.size(); ++i) {
                ...
                const auto inE_set = view_->getIncomingEdgesStable(b);
                std::vector<EdgePtr> inE_vec(inE_set.begin(), inE_set.end());
                std::sort(inE_vec.begin(), inE_vec.end(), [](const EdgePtr& a, const EdgePtr& b){
                    return a->getEdgeKey() < b->getEdgeKey();
                });

                for (const EdgePtr& pe : inE_vec) {
                    if (pe->pruned || !(pe->getProbability() < 1.0)) continue;
                    bridge.push_back(mapEdgeId(pe->getId()));
                }
                if (bridge.size() >= 2) push_event(bridge, P_.w_bridge);
            }
        }

        // BRIDGE (type 2, optional)
        if (P_.enable_bridge_shared_input) {
            std::map<UntypedTuple, std::vector<int>> fact_to_ruleflips;
            for (const EdgePtr& e : sorted_edges) {
                if (e->pruned || !(e->getProbability() < 1.0)) continue;
                ...
                for (size_t i = 0; i < inputs.size(); ++i) {
                    const NodePtr b = inputs[i];
                    const bool neg  = (i < negs.size()) ? negs[i] : false;
                    if (!b || neg || !b->isFact || b->pruned || !(b->getProbability() < 1.0)) continue;
                    fact_to_ruleflips[b->getTuple()].push_back(rvid);
                }
            }

            for (const auto& kv : fact_to_ruleflips) {
                const auto& flips = kv.second;
                if (flips.size() >= 2) {
                    std::vector<int> bridge = flips;
                    // Add the shared fact's var_id, which requires the new reverse map.
                    if (tuple_to_fact_var_.count(kv.first)) {
                        bridge.push_back(tuple_to_fact_var_.at(kv.first));
                    }
                    if(bridge.size() >= 2) push_event(bridge, P_.w_bridge);
                }
            }
        ...
        // Final Sort based on the canonical key.
        std::sort(events_.begin(), events_.end(), [](const Event& a, const Event& b) {
            return a.stable_key < b.stable_key;
        });

//        std::cout << std::fixed << std::setprecision(8);
//        std::cout << "[Sorted Events] |events|=" << events_.size() << "\n";
//        int i = 0;
//        for (const auto& e : events_) {
//            std::cout << i++ << " " << " Event weight=" << e.weight << " vars=";
//            for (int v : e.var_ids) {
//                if (fact_var_to_tuple_.count(v)) {
//                    std::cout << "FACT " << " (" << fact_var_to_tuple_.at(v).toString() << ") ";
//                } else {
//                    std::cout << "RULE " << " (" << std::get<1>(rule_var_to_key_.at(v)).toString() << ") ";
//                }
//            }
//            std::cout << "\n";
//        }
    ...
    double totalSpan(const std::vector<int>& order) const {
        ...
        for (const auto& e : events_) {
            ...
            for (int v : e.var_ids) {
                auto it = pos.find(v);
                if (it == pos.end()) assert (false); // shouldn't happen
    ...
    void forceFree() {
        ...
        for (int it = 0; it < P_.force_iters; ++it) {
            ...
            for (size_t ei = 0; ei < events_.size(); ++ei) {
                ...
                for (int v : events_[ei].var_ids) {  // TODO
                    auto itp = pos.find(v);
            ...
            std::stable_sort(order_.begin(), order_.end(),
                [&](int a, int b) {
                    ...
                    if (isNearlyEqual(ta, tb)) {
                        std::variant<UntypedTuple, EdgeKey> key1;
                        if (fact_var_to_tuple_.count(a)) key1 = fact_var_to_tuple_.at(a);
                        else                           key1 = rule_var_to_key_.at(a);
                        std::variant<UntypedTuple, EdgeKey> key2;
                        if (fact_var_to_tuple_.count(b)) key2 = fact_var_to_tuple_.at(b);
                        else                           key2 = rule_var_to_key_.at(b);
                        return key1 < key2;
                    }
                    return ta < tb;
                });
