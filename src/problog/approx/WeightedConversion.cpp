...
UnweightedCNFResult convertWeightedToUnweighted(WeightedCNFInput input, const ConversionConfig& config) {
    ...
    try {
        ...
        std::unordered_map<uint32_t, QuantizedWeight> quantizedWeights;
        quantizedWeights.reserve(varsToEncode.size());
        std::deque<int> quantizedUnitQueue;
        ...
        for (uint32_t var : varsToEncode) {
            ...
            if (q.bitPrecision == 0) {
                if (isOne(q.representedWeight)) {
                    quantizedUnitQueue.push_back(static_cast<int>(var));
                } else if (isZero(q.representedWeight)) {
                    quantizedUnitQueue.push_back(-static_cast<int>(var));
                } else {
                    out.report.valid = false;
                    out.report.message =
                            "Quantization collapsed to a non-Boolean weight; internal error";
                    return out;
                }
            }
            quantizedWeights.emplace(var, q);
        }

        if (!quantizedUnitQueue.empty()) {
            // Low-precision rounding can collapse near-0/near-1 weights to exact
            // Booleans. Treat those as forced assignments instead of failing.
            std::unordered_map<uint32_t, bool> assignment;
            auto quantizedLiteralWeight = [&quantizedWeights](int lit) -> long double {
                const uint32_t var = static_cast<uint32_t>(std::abs(lit));
                const auto it = quantizedWeights.find(var);
                if (it == quantizedWeights.end()) {
                    return 1.0L;
                }
                const long double posWeight = it->second.representedWeight;
                return (lit > 0) ? posWeight : (1.0L - posWeight);
            };

            while (!quantizedUnitQueue.empty()) {
                const int lit = quantizedUnitQueue.front();
                quantizedUnitQueue.pop_front();
                ...
                if (it != assignment.end()) {
                    if (it->second != value) {
                        out.report.unsat = true;
                        out.report.valid = true;
                        out.report.message = "Conflicting assignments after quantization";
                        out.multiplier = 0.0L;
                        out.divideExp = 0;
                        out.numVars = input.numVars;
                        out.samplingSet.assign(samplingSet.begin(), samplingSet.end());
                ...
                const long double litWeight = quantizedLiteralWeight(lit);
                if (litWeight < -kEps) {
                    throw std::runtime_error("Encountered negative literal weight after quantization");
                }
                ...
                for (const auto& clause : clauses) {
                    ...
                    if (canonical.empty()) {
                        out.report.unsat = true;
                        out.report.valid = true;
                        out.report.message = "Quantization simplification produced empty clause";
                        out.multiplier = 0.0L;
                        out.divideExp = 0;
                        out.numVars = input.numVars;
                        out.samplingSet.assign(samplingSet.begin(), samplingSet.end());
                    ...
                    if (canonical.size() == 1) {
                        quantizedUnitQueue.push_back(canonical.front());
                    }
                    simplified.push_back(std::move(canonical));
                }
                clauses.swap(simplified);
        ...
        out.report.weightedVarsAfter = static_cast<uint32_t>(quantizedWeights.size());
