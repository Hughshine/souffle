...
class ExactClear : public RelationStatement {
public:
    ExactClear(std::string rel) : RelationStatement(NK_ExactClear, rel) {}

    ExactClear* cloning() const override {
        return new ExactClear(relation);
    }

    static bool classof(const Node* n) {
        return n->getKind() == NK_ExactClear;
    }

protected:
    void print(std::ostream& os, int tabpos) const override {
        os << times(" ", tabpos);
        os << "EXACTCLEAR " << relation << std::endl;
    }
};
