...
public:
    GuardedInsert(std::string rel, VecOwn<Expression> expressions, Own<Condition> condition = mk<True>(), std::size_t clauseID = -1, std::string clauseStr = "CLAUSE")
            : Insert(NK_GuardedInsert, rel, std::move(expressions), std::move(clauseID), std::move(clauseStr)), condition(std::move(condition)) {}
