...
{
	Logger logger(R"_(@t-nonrecursive-relation;a;test.dl [7:7-7:8];)_",iter, [&](){return rel_a_454ddee488c1ed6b->size();});
signalHandler->setMsg(R"_(a(X,Y) :- 
   b(X,Z),
   c(Y,Z).
in file  [1:1-1:1])_");
{
	Logger logger(R"_(@t-nonrecursive-rule;a; [1:1-1:1];a(X,Y) :- \n   b(X,Z),\n   c(Y,Z).;)_",iter, [&](){return rel_a_454ddee488c1ed6b->size();});
...
in file  [1:1-1:1])_");
{
	Logger logger(R"_(@t-nonrecursive-rule;res; [1:1-1:1];res(x) :- \n   a(@abdul0,x),\n   @abdul0 = 1.;)_",iter, [&](){return rel_res_f46f91340698127b->size();});
...
namespace  souffle {
using namespace souffle;
class Sf_compute_: public SouffleProgram {
public:
 Sf_compute_(std::string pf = "profile.log");
 ~Sf_compute_();
void run();
void runInc();
...
namespace  souffle {
using namespace souffle;
 Sf_compute_::Sf_compute_(std::string pf):
...
ProfileEventSingleton::instance().setOutputFile(profiling_fname);
 ...
 Sf_compute_::~Sf_compute_(){
}

void Sf_compute_::runFunction(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
...
ProfileEventSingleton::instance().makeConfigRecord("relationCount", std::to_string(20));{
...
void Sf_compute_::runFunctionInc(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
...
void Sf_compute_::run(){
runFunction("", "", false, false);
}

void Sf_compute_::runInc(){
runFunctionInc("", "", false, false);
}

void Sf_compute_::runAll(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
runFunction(inputDirectoryArg, outputDirectoryArg, performIOArg, pruneImdtRelsArg);
}

void Sf_compute_::runAllInc(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
runFunctionInc(inputDirectoryArg, outputDirectoryArg, performIOArg, pruneImdtRelsArg);
}

void Sf_compute_::printAll([[maybe_unused]] std::string outputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","X"},{"auxArity","0"},{"inc-delete","false"},{"inc-insert","false"},{"incDelta","false"},{"name","res"},{"operation","output"},{"output-dir","./output"},{"params","{\"records\": {}, \"relation\": {\"arity\": 1, \"params\": [\"X\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 1, \"types\": [\"i:number\"]}}"}});
...
void Sf_compute_::loadAll([[maybe_unused]] std::string inputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","X\tZ"},{"auxArity","0"},{"cache","true"},{"fact-dir","./input"},{"inc-delete","false"},{"inc-insert","false"},{"incDelta","false"},{"name","b"},{"operation","input"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"X\", \"Z\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf_compute_::loadAllExcept([[maybe_unused]] std::string inputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","X\tZ"},{"auxArity","0"},{"cache","true"},{"fact-dir","./input"},{"inc-delete","false"},{"inc-insert","false"},{"incDelta","false"},{"name","b"},{"operation","input"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"X\", \"Z\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
if (!inputDirectoryArg.empty()) {directiveMap["fact-dir"] = inputDirectoryArg;}
...
SymbolTable& Sf_compute_::getSymbolTable(){
return symTable;
}

RecordTable& Sf_compute_::getRecordTable(){
return recordTable;
}

void Sf_compute_::setNumThreads(std::size_t numThreadsValue){
...
void Sf_compute_::executeSubroutine(std::string name,const std::vector<RamDomain>& args,std::vector<RamDomain>& ret){
if (name == "a") {
stratum_a_f1f7b26a4b108ce3.run(args, ret);
return;}
if (name == "b") {
...
void Sf_compute_::dumpFreqs(){
}

} // namespace  souffle
std::vector<std::pair<UntypedTuple,bool>> evidences;
namespace souffle {
SouffleProgram *newInstance_compute_(){return new  souffle::Sf_compute_;}
SymbolTable *getST_compute_(SouffleProgram *p){return &reinterpret_cast<souffle::Sf_compute_*>(p)->getSymbolTable();}
} // namespace souffle
...
souffle::Sf_compute_ obj(opt.getProfileName());
...
souffle::ProfileEventSingleton::instance().makeConfigRecord("version", "efb25a3a4");
debugger.startTurn();
...
const Atom atom_2_1 = Atom{"a", {SymbolicField::makeVariable("@abdul0"), SymbolicField::makeVariable("x"), }};
const Rule rule2 = Rule(2,rule2_head, {atom_2_1}, {"@abdul0", "x"}, 1.000000, 0, 0, false);
...
namespace  souffle {
using namespace souffle;
class factory_Sf_compute_: souffle::ProgramFactory {
public:
...
namespace  souffle {
using namespace souffle;
souffle::SouffleProgram* factory_Sf_compute_::newInstance(){
return new  souffle::Sf_compute_();
}

 factory_Sf_compute_::factory_Sf_compute_():
souffle::ProgramFactory("compute_"){
}
...
extern "C" {
souffle::factory_Sf_compute_ __factory_Sf_compute__instance;
}
#endif
} // namespace souffle
