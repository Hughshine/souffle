...
- Compile with `--full-only` to disable incremental code paths. CUDD uses adaptive
  dynamic reordering (not a fixed sift-only policy).
...
```
This uses `souffle --full-only ...`; `--online` is ignored when `--full-only` is set.
