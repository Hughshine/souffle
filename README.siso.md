...
> The current detector uses fast-path patterns only; the dominance-based detector is retained but disabled. For rewrite behavior and performance notes, see `README.rewrite.impl.md`.
...
## Scope
- Detection algorithm only (GraphAnalyzer); rewrite application lives in `README.rewrite.impl.md`.
