...
- `total_ms`, `const_ms`, `preConfig_ms`, `depGraph_ms`, `baseInit_ms`, `cycles_ms`, `rounds`
...
- `input_literal_calls`, `input_literal_missing`, `input_literal_ms`
...
- `delete_condition`: conditioning on deleted non-deterministic facts  
  ...
  `delta_live_nodes`, `live_nodes`
- `delete_overdelete`: set det-impacted formulas to False  
  `ms`, `det_imp_nodes`, `det_imp_edges`, `delta_live_nodes`, `live_nodes`
- `delete_varorder`: variable ordering cleanup  
  `ms`, `collect_ms`, `postprocess_ms`, `dump_ms`, `post_del_enabled`, `deleted_vars`
...
- `insert_init_nodes`: finer node init breakdown  
  `fact_true_ms`, `fact_var_ms`, `fact_weight_ms`, `nonfact_ms`,
  `fact_true`, `fact_var`, `nonfact`
...
- `tag`: `full_cyclewise`, `full_ondemand`, `inc_insert`, or `inc_regional_insert`
  (empty tags print as `unknown`)
...
- Emitted once per `createVar(...)` call during preConfig (tagged `full_cyclewise`,
  `full_ondemand`, `inc_insert`, or `inc_regional_insert`).
- Fields:
